...
then
  echo "Usage: fetch-model.sh \"FilePattern\" DestinationDirectory [MODEL_TAG]"
  echo "Example: fetch-model.sh \"^(identifier-mobilenetv2-c9\..+|matting-fba\..+|shadowgen256_car\..+|trimap513-deeplab-res2net\..+)$\" ./models/ 1.9.2"
  exit -1
fi
