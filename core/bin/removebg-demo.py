...
def handle_args():
    ...
    parser.add_argument(
        "--disable_confidence_thresh", action="store_true", help="disables the bad-quality filtering"
    )
...
def demo(args):
    ...
    if args.path:
        ...
        for file in files:
            ...
            try:
                im_tr_bgr, im_tr_alpha = removebg(
                    im_tr,
            ...
            if not args.evaluate:
                ...
                eval_data[os.path.dirname(file)][os.path.basename(file)] = np.sqrt(
                    (1.0 * im_alpha_gt - 1.0 * image.im_alpha) ** 2
                ).mean()
