...
from kaleido.training.distributed.utilities import (
    callbacks_distributed,
...
def main():
    ...
    parser.add_argument(
        "--danni_metadata_path", required=False, type=str, help="JSON file with prefetched danni paths."
    )
    ...
    parser.add_argument(
        "--scale_lr_ddp",
        action="store_true",
        help="When using distributed training, multiply the learning rate with the number of nodes",
    )
    ...
    if args.danni_user is not None:
        ...
        loader = DanniLoader(
            ...
            root="danni-data/",
            limit=1000,
            max_pages=args.danni_max_pages,
        )
    else:
    ...
    if is_distributed_enabled():
        ...
        train_sampler = KaleidoDistributedSampler(
            train_dataset, shuffle=True, num_replicas=num_replicas, rank=rank, drop_last=True
        )
        valid_sampler = KaleidoDistributedSampler(
            valid_dataset, shuffle=False, num_replicas=num_replicas, rank=rank, drop_last=True
        )
    ...
    valid_loader = torch.utils.data.DataLoader(
        valid_dataset, batch_size=8, shuffle=False, sampler=valid_sampler, num_workers=args.workers
    )
    ...
    configure_cloud_training(
        ...
        checkpoint_names=["last.ckpt", "best.ckpt"],
    )
