...
def main():
    ...
    def callback(ch, method, props, body):
        ...
        try:
            ...
            print('[{}] {:10} ({:.2f}s) | megapixels: {}, mode: {}, dpi: {}, icc: {}, has alpha: {}, exif: {}'.format(props.correlation_id, 'decoding', times[-1], megapixels, image.mode_original, image.dpi, 'yes' if image.icc else 'no', 'yes' if image.im_alpha is not None is not None else 'no', 'none' if image.exif_rot is None else image.exif_rot))
            t = time.time()
            ...
            times.append(time.time() - t)
            print('[{}] {:10} ({:.2f}s) | {}x{} -> {}x{} (crop: {})'.format(props.correlation_id, 'preproc', times[-1], image.width, image.height, im_cv.shape[1], im_cv.shape[0], crop))
            ...
            print('[{}] {:10} ({:.2f}s) | class: {}'.format(props.correlation_id, 'processing', times[-1], shared_data['api']))
            ...
            if shared_data['api'] == 'car':
                image.fill_holes(200 if semitransparency else 255, mode='car', average=semitransparency, im_rgb_precolorcorr=im_rgb_precolorcorr)

            elif shared_data['api'] == 'car_interior':
                image.fill_holes(200 if semitransparency else 0, mode='all', average=True, im_rgb_precolorcorr=im_rgb_precolorcorr)
            ...
            if crop:
                ...
                kwargs = {}
                if crop_margin:
                    kwargs['margins'] = [crop_margin[b'top'], crop_margin[b'right'], crop_margin[b'bottom'], crop_margin[b'left']]
                    kwargs['absolutes'] = [not crop_margin[b'top_relative'], not crop_margin[b'right_relative'], not crop_margin[b'bottom_relative'], not crop_margin[b'left_relative']]
                    kwargs['clamp'] = 500
            ...
            times.append(time.time() - t)
            print('[{}] {:10} ({:.2f}s) | bg_color: {}, shadow: {}'.format(props.correlation_id, 'postproc', times[-1], bg_color, 'yes' if shadow else 'no'))
            t = time.time()
    ...
    def worker():
        print('Initializing rabbitmq connection. host "{}", port "{}", queue "{}"'.format(os.environ['RABBITMQ_HOST'], os.environ['RABBITMQ_PORT'], os.environ['REQUEST_QUEUE']))

        try:
            connection = pika.BlockingConnection(pika.ConnectionParameters(os.environ['RABBITMQ_HOST'], os.environ['RABBITMQ_PORT'], '/', pika.PlainCredentials(os.environ['RABBITMQ_USER'], os.environ['RABBITMQ_PASSWORD'])))
