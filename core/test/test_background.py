...
def test_bg_im_png(req_fn):
    (
        ...
        _,
        _,
    ) = req_fn("data/CMYK.jpg", {"format": "png", "size": "full", "channels": "alpha"})

    im_bg = Image.open("data/RGB.png")
    im_bg = im_bg.resize((im_alpha.width, im_alpha.height))

    im_color, im_in, headers, status_code = req_fn(
        "data/CMYK.jpg", {"format": "png", "size": "full", "bg_image_file": pil_to_bytes(im_bg)}
    )
...
def test_bg_im_jpg(req_fn):
    ...
    im_color, im_in, headers, status_code = req_fn(
        "data/CMYK.jpg", {"format": "jpg", "size": "full", "bg_image_file": pil_to_bytes(im_bg)}
    )
...
def test_bg_im_zip(req_fn):
    ...
    im, im_in, headers, status_code = req_fn(
        "data/CMYK.jpg", {"format": "zip", "size": "full", "bg_image_file": pil_to_bytes(im_bg)}
    )
...
def test_bg_color_jpg(req_fn):
    im_color, im_in, headers, status_code = req_fn(
        "data/RGB.png", {"format": "jpg", "size": "full", "bg_color": "ffaa33"}
    )
...
def test_bg_color_zip(req_fn):
    im, im_in, headers, status_code = req_fn(
        "data/RGB.png", {"format": "zip", "size": "full", "bg_color": "ffaa33"}
    )
...
def test_bg_color_transparent_zip(req_fn):
    im, im_in, headers, status_code = req_fn(
        "data/RGB.png", {"format": "zip", "size": "full", "bg_color": "ffaa3311"}
    )
    im_color, im_alpha = im
