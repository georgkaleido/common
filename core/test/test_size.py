...
def test_small(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 0.5), {'format': 'jpg', 'size': 'small'})
    check_sizes(im_in, im, 0.25, headers)

def test_preview(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 0.5), {'format': 'jpg', 'size': 'preview'})
    check_sizes(im_in, im, 0.25, headers)

def test_regular(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 0.5), {'format': 'jpg', 'size': 'regular'})
    check_sizes(im_in, im, 0.25, headers)

def test_preview_lower(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 0.2), {'format': 'jpg', 'size': 'preview'})
    check_sizes(im_in, im, 0.2, headers)

# test medium (0.25 - 1.5mp)

def test_medium_lower(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 0.2), {'format': 'jpg', 'size': 'medium'})
    check_sizes(im_in, im, 0.2, headers)

def test_medium(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 1.1), {'format': 'jpg', 'size': 'medium'})
    check_sizes(im_in, im, 1.1, headers)

def test_medium_upper(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 2.0), {'format': 'jpg', 'size': 'medium'})
...
def test_full_lower(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 3.5), {'format': 'jpg', 'size': 'full'})
    check_sizes(im_in, im, 3.5, headers)

def test_full(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 5.0), {'format': 'jpg', 'size': 'full'})
...
def test_full_upper(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 27.0), {'format': 'jpg', 'size': 'full'})
    check_sizes(im_in, im, 25, headers)

# test auto

def test_auto_small(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 0.2), {'format': 'jpg', 'size': 'full'})
    check_sizes(im_in, im, 0.2, headers)

def test_auto_medium(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 0.5), {'format': 'jpg', 'size': 'full'})
    check_sizes(im_in, im, 0.5, headers)

def test_auto_hd(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 2.0), {'format': 'jpg', 'size': 'full'})
...
def test_full_png(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 27.0), {'format': 'png', 'size': 'full'})
    check_sizes(im_in, im, 10, headers)

def test_full_zip(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 27.0), {'format': 'zip', 'size': 'full'})
    check_sizes(im_in, im[0], 25, headers)
...
def test_image_too_large(req_fn):
    im, im_in, headers, status_code = req_fn(scaled_im('data/CMYK.jpg', 51.0), {'format': 'jpg', 'size': 'full'})
